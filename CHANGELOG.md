...
- `fugue-ppl` is now a `path = "../fugue"` dependency (with `version = "0.1.0"` retained for the published release) onto the sibling `fugue` checkout, so local edits to `fugue` are picked up by `fugue-evo` builds/tests instead of silently resolving to the last crates.io release; verified against the current sibling API with a clean `cargo check --all-targets`. README documents the sibling-checkout requirement and a `[patch]` fallback for consumers without it (EV-30).
...
- `proptest`'s dev-dependency chain unconditionally pulls in rand 0.9/rand_core 0.9/rand_chacha 0.9 alongside our own direct rand 0.8 stack; confirmed via `cargo tree -i rand@0.9.2` etc. that this holds even pinned back to proptest 1.7.0, and that the upstream `fugue` sibling has the identical duplication despite the same `proptest = "1.0"` pin — documented as upstream-forced in `Cargo.toml` rather than "fixed" by a version bump (EV-74).
