...
- `fugue-ppl` is deliberately kept as the published `fugue-ppl = "0.1.0"` crates.io dependency rather than a `path = "../fugue"` dependency onto the sibling checkout: `fugue-evo` and `fugue` are developed and audited independently, and hard-wiring the build to `../fugue`'s working tree would make `fugue-evo`'s build only as stable as whatever `fugue` happens to look like on disk. A new README "Development" section documents this rationale and shows the `[patch.crates-io] fugue-ppl = { path = "../fugue" }` override a developer adds to their own local, uncommitted top-level manifest to pick up sibling edits during co-development (EV-30).
